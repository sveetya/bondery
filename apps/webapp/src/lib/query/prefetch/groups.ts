...
export async function fetchGroupsList(
  ...
  await fetchQueryOrThrow(queryClient, groupKeys.list(params), () => getGroupsListServer(params));
}
...
export async function fetchGroupDetail(queryClient: QueryClient, groupId: string): Promise<void> {
  await fetchQueryOrThrow(queryClient, groupKeys.detail(groupId), () =>
    getGroupDetailServer(groupId),
  );
}
