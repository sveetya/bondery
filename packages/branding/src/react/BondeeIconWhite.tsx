import * as React from "react";
import type { SVGProps } from "react";
const BondeeIconWhite = (props: SVGProps<SVGSVGElement>) => (
  <svg
    width={40}
    height={40}
    viewBox="0 0 40 40"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <rect width={40} height={40} rx={8} fill="url(#gradient)" />
    <path
      d="M12 14C12 12.8954 12.8954 12 14 12H18C19.1046 12 20 12.8954 20 14V18C20 19.1046 19.1046 20 18 20H14C12.8954 20 12 19.1046 12 18V14Z"
      fill="white"
      opacity={0.9}
    />
    <path
      d="M22 14C22 12.8954 22.8954 12 24 12H28C29.1046 12 30 12.8954 30 14V18C30 19.1046 29.1046 20 28 20H24C22.8954 20 22 19.1046 22 18V14Z"
      fill="white"
      opacity={0.9}
    />
    <path
      d="M12 24C12 22.8954 12.8954 22 14 22H18C19.1046 22 20 22.8954 20 24V28C20 29.1046 19.1046 30 18 30H14C12.8954 30 12 29.1046 12 28V24Z"
      fill="white"
      opacity={0.9}
    />
    <path
      d="M22 24C22 22.8954 22.8954 22 24 22H28C29.1046 22 30 22.8954 30 24V28C30 29.1046 29.1046 30 28 30H24C22.8954 30 22 29.1046 22 28V24Z"
      fill="white"
      opacity={0.9}
    />
    <circle cx={20} cy={20} r={2} fill="white" />
    <defs>
      <linearGradient id="gradient" x1={0} y1={0} x2={40} y2={40} gradientUnits="userSpaceOnUse">
        <stop stopColor="#7C3AED" />
        <stop offset={1} stopColor="#C026D3" />
      </linearGradient>
    </defs>
  </svg>
);
export { BondeeIconWhite };
