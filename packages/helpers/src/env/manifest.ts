...
export const ENV_GROUP_GUIDES: Readonly<Record<string, readonly string[]>> = {
  ...
  Auth: [
    ...
    "  Then run: npm run provision:oauth-clients",
  ],
  Database: [
...
export type SyncTargetConfig = {
  ...
  devFile: string;
  /** Relative to repo root — committed example regenerated by `npm run env:examples` */
  exampleFile: string;
  /** Optional alternate example for production shape */
  productionExampleFile?: string;
  /** Relative to repo root — where `npm run env:production` writes */
  productionFile?: string;
};

/** Where `npm run env:development` / `npm run env:production` write per-app local files */
