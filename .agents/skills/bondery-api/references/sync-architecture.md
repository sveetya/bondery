...
| **Web** | BFF ticket + same-origin WS (`/api/sync/ws` proxy) → TanStack Query invalidation by `affectedTables` |
...
- [ ] `npm run check-sync-patterns --workspace=mobile` passes locally
