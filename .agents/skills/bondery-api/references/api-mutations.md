...
- [ ] Create returns `201` + full resource object (not just `{ id }`)
- [ ] Update returns `200` + full updated resource object
- [ ] Client uses response body directly — no follow-up GET (unless bulk/async exception)
- [ ] Webapp invalidates TanStack Query keys; mobile tier-1 uses `submitSyncMutation`
- [ ] Response schema documents resource-keyed shape (`{ contact }`, not `{ data }`)
