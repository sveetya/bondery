# References

| File | Topic |
|------|--------|
| [architecture.md](./architecture.md) | Mirror model, ADR |
| [webhook-events.md](./webhook-events.md) | Event → action map |
| [stripe-product-model.md](./stripe-product-model.md) | Products, prices, checkout |
| [access-control.md](./access-control.md) | Premium grace period |
| [env-vars.md](./env-vars.md) | Configuration |
| [api-routes.md](./api-routes.md) | Fastify routes |
| [client-integration.md](./client-integration.md) | Webapp embedded checkout |
| [trial-ending-email.md](./trial-ending-email.md) | Trial email billing trigger (template → bondery-emails) |
