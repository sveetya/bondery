...
| Trigger | Commands |
|---------|----------|
| Prisma schema/migrations | `npm run check:types -w @bondery/db`, `npm run db:generate` |
| Release/CI parity | `npm run release-migrate` (requires Postgres — see `ci-parity.md`) |
...
| Sync/Redis | `npm run test:api:sync` |
...
| Trigger | Commands |
|---------|----------|
| Template change | `npm run compile -w @bondery/emails` |
| Preview (manual) | `npm run dev:emails` |
...
| Shared packages | `npm run build:packages` or `npm run build -w @bondery/schemas` |
