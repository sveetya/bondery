...
|-------|----------|---------|---------|------|----------|-----|-------------|-------------|
| Share contact | `ShareContactEmail` | User `POST` share contact API | `{sender} shared a contact with you • {name}` | `Bondery <address>` | Sender email | Sender email | None (user-initiated) | bondery-emails |
...
| Email | Template | Sender | Trigger |
|-------|----------|--------|---------|
| Share contact | `packages/emails/src/templates/ShareContactEmail.tsx` | `apps/api/src/services/contacts/share.ts` | Contact share route |
| Reminder digest | `packages/emails/src/templates/ReminderDigestEmail.tsx` | `apps/api/src/services/notifications/reminder-digest.ts` | `apps/api/src/services/notifications/reminder-dispatch.ts` |
| Trial ending | `packages/emails/src/templates/TrialEndingEmail.tsx` | `apps/api/src/services/notifications/trial-ending.ts` | `apps/api/src/services/billing/webhook-handlers/trial-ending.ts` |
| Account deleted | `packages/emails/src/templates/AccountDeletedEmail.tsx` | `apps/api/src/services/notifications/account-deleted.ts` | `apps/api/src/lib/auth/teardown-user.ts` |
| Welcome | `packages/emails/src/templates/WelcomeEmail.tsx` | `apps/api/src/services/notifications/welcome.ts` | `apps/api/src/lib/auth/index.ts` (`databaseHooks.user.create.after`) |
| Feedback | `packages/emails/src/templates/FeedbackEmail.tsx` | `apps/api/src/services/notifications/feedback.ts` | `apps/api/src/routes/me/feedback/index.ts` |
