...
Secrets Manager (AWS SM / Vault)
    ...
    → api / webapp / migrate containers inherit env
```

Store one JSON object per environment, keys matching the manifest canonical names. The `migrate` service needs the same OAuth and database secrets as `api` because `release-migrate` provisions OAuth clients after `prisma migrate deploy`.

## Secret inventory

| Secret | Used by | Rotation impact |
| --- | --- | --- |
| `BONDERY_PRIVATE_BETTER_AUTH_SECRET` | API | **High** — invalidates all Better Auth sessions and issued tokens until users re-login |
...
| `BONDERY_PRIVATE_POSTGRES_PASSWORD` | DB, API, migrate | **High** — requires coordinated DB user password change + env update + restart |
...
### `BONDERY_PRIVATE_BETTER_AUTH_SECRET`
...
3. Update the secret in your secrets store / Dokploy env.
4. Redeploy `api` (and `webapp` if it shares the deploy bundle).
5. Verify: `GET https://<api-domain>/status` and a test login on webapp + mobile.
6. Record rotation date in your password manager.
...
3. Redeploy `migrate` (or run `npm run provision-oauth-clients` against production) so the API's stored hash updates.
...
3. Restart `db` (if needed), then `migrate`, `api`, and any job workers.
...
3. Run provisioning (`release-migrate` on deploy, or `provision-oauth-clients` manually).
...
Store them in your secrets manager, map into [`deploy/bondery/.env`](../../deploy/bondery/.env.example), then deploy. The `migrate` service runs `release-migrate`, which applies schema migrations and provisions OAuth clients automatically.
