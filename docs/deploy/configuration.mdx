...
Operators configure the stack with **`BONDERY_*`** names. Copy [`deploy/bondery/.env.example`](../../deploy/bondery/.env.example) to `.env` — that file is generated from [`packages/helpers/src/env/manifest.ts`](../../packages/helpers/src/env/manifest.ts) (`deployExample` metadata). Do not hand-edit the example; change the manifest and run `npm run env -- --write-examples`.
...
| Variable | Purpose |
|---|---|
| `BONDERY_PRIVATE_S3_ENDPOINT` | Internal S3 endpoint (e.g. `http://seaweedfs-s3:8333` in Compose) |
...
On every deploy, `api` `pre_start` runs `release-migrate` (schema, functions, OAuth clients, platform admins) then `ensure-storage-buckets` before the main API process starts. See [Get started](./get-started.mdx) and [Dokploy deployment](../contributing/dokploy.mdx).
