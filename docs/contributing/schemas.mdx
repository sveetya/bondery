...
`pnpm run check:schemas-imports -w webapp` enforces these rules in CI.
...
- `scripts/check-contracts-openapi-examples.ts` — validates fixtures against runtime Zod schemas
- `scripts/check-contracts-no-init-cycles.mjs` — blocks `#openapi/` imports in runtime paths
...
After adding a new public subfolder barrel, run `pnpm run sync-exports` from the repo root.
