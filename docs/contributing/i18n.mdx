...
|---------|------------------|
| `pnpm run check:i18n` | All i18n CI gates below (umbrella) |
| `pnpm run check:i18n:structure` | Locale file parity, manifest rules, Languages exonyms |
| `pnpm run check:i18n:types` | Generated i18next-cli types are current |
| `pnpm run check:i18n:usage` | Every key used in code (including `TypedTrans` `i18nKey`) exists in all locales; cs/de key parity vs en |
| `pnpm run check:i18n:lint` | No unexpected hardcoded UI strings (webapp + mobile) |
