...
**CI:** Path-filtered PRs and pushes to `main` run `website-build` in [`.github/workflows/verify.yml`](../../.github/workflows/verify.yml) (pruned production `next build`). Push to `release` runs [`.github/workflows/deploy-website.yml`](../../.github/workflows/deploy-website.yml) (Docker build-push → smoke → optional Dokploy webhook); image tags `:production` + `:sha-<short>`. Secret `BONDERY_OPS_DOKPLOY_WEBSITE_DEPLOY_WEBHOOK` for redeploy.
