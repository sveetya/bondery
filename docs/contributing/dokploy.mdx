...
Self-hosters use **only** [`deploy/bondery`](../../deploy/bondery/) (api + webapp + redis + Postgres). Marketing lives in ops and is not part of the self-host distribution. Guides: [`deploy/bondery/README.md`](../../deploy/bondery) · [`docs/deploy/get-started.mdx`](../deploy/get-started.mdx) · [`deploy/ops/README.md`](../../deploy/ops).
...
| `api` | `GET /health/live` | `GET /health/ready` | Process up / Postgres, SeaweedFS gateway `/status`, required S3 buckets (`HeadBucket`), Redis, SMTP (critical — 503 when unhealthy); optional integrations degrade only |
...
After `api-*.*.*` / `webapp-*.*.*` tags build and push `:production`, [`.github/workflows/release-api.yml`](../../.github/workflows/release-api.yml) and [`.github/workflows/release-webapp.yml`](../../.github/workflows/release-webapp.yml) POST to `BONDERY_OPS_DOKPLOY_SERVICES_DEPLOY_WEBHOOK` (GitHub **variable**) with `refs/heads/release`. Set the Dokploy Compose app branch to **`release`**. Leave the variable empty to redeploy manually.
...
**CI:** Path-filtered PRs and pushes to `main` run `website-build` in [`.github/workflows/verify.yml`](../../.github/workflows/verify.yml) (pruned production `next build`). Push to `release` runs [`.github/workflows/deploy-website.yml`](../../.github/workflows/deploy-website.yml) (Docker build-push → smoke → optional Dokploy webhook); image tags `:production` + `:sha-<short>`. Repository variable `BONDERY_OPS_DOKPLOY_OPS_DEPLOY_WEBHOOK` triggers redeploy (`refs/heads/release`).
