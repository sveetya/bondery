...
|-------|----------|-----------|----------------|
| Production | `usebondery.com` | `:production` | `release` |
| Beta | `beta.usebondery.com` | `BONDERY_INFRA_WEBSITE_IMAGE_TAG=beta` (Dokploy UI) | `main` |

**CI:** Path-filtered PRs and pushes to `main` run `website-build` in [`.github/workflows/verify.yml`](../../.github/workflows/verify.yml) (pruned production `next build`). [`stage-images.yml`](../../.github/workflows/stage-images.yml) on `main` tags website `:beta` + `:sha-<short>` and notifies the staging website webhook. Push to `release` runs [`.github/workflows/deploy-website.yml`](../../.github/workflows/deploy-website.yml) (Docker build-push → smoke → optional Dokploy webhook from Infisical production); image tags `:production` + `:sha-<short>`. Production CD stays on `deploy-website.yml`. Infisical key `BONDERY_OPS_DOKPLOY_WEBSITE_DEPLOY_WEBHOOK` triggers production redeploy (`refs/heads/release`).
