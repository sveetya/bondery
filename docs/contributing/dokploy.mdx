...
After unified `vX.Y.Z` release tags (or legacy `api-*` / `webapp-*`), GitHub Actions fetches **production Infisical** and POSTs to `BONDERY_OPS_DOKPLOY_SERVICES_DEPLOY_WEBHOOK` with `refs/heads/release`. Set the Dokploy Compose app branch to **`release`**. Leave the webhook unset in Infisical to redeploy manually.
