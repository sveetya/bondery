...
- **In-flight entries** — edit [`docs/changelog/unreleased.mdx`](../../changelog/unreleased.mdx) (`title` + `hidden: true`; not in `meta.json`).
- **Shipped releases** — one file per version under `docs/changelog/releases/X.Y.Z.mdx` (`title: "Changelog X.Y.Z"`, `hidden: true`), imported newest-first by [`docs/changelog.mdx`](../../changelog.mdx).
- Do not add release partials to `meta.json`; the public page stays a single sidebar entry.
