...
This guide walks you through setting up the full Bondery development environment. Follow the dependency chain: **Postgres → migrations → API → apps**.
...
| [Docker](https://www.docker.com/) | latest | Local Postgres and Redis |
...
The local Postgres instance is the foundation everything else connects to. Start it before running migrations or the API.
...
# Set BONDERY_PRIVATE_POSTGRES_PASSWORD to match root .env.local
docker compose -f docker-compose.dev-db.yml up -d
```

This starts PostGIS on host port **54322** with database name `bondery`.
...
| Command | Description |
|---|---|
| `npm run db:migrate:deploy -w @bondery/db` | Apply pending migrations (normal local setup, CI, self-host) |
...
| `npm run db:functions -w @bondery/db` | Apply raw SQL functions (self-host first boot) |
| `npm run provision-oauth-clients:dev -w api` | Upsert first-party OAuth clients after migrations |

For a full auth walkthrough, see [Authentication](./auth.mdx).

For a fresh local DB, re-run `npm run db:migrate:deploy -w @bondery/db` (or `prisma migrate reset` inside `packages/db` when you intentionally want to wipe data — then re-run provisioning).
...
Redis powers **rate limiting**, **mobile sync wake** (pub/sub), and **WebSocket ticket** storage in the API.

| Mode | `BONDERY_PRIVATE_REDIS_URL` | When to use |
|------|---------------------|-------------|
| **Local Docker (default)** | `redis://127.0.0.1:26636` | Normal local API work |
| **In-memory** | empty / unset | Quick feature work without Redis |
...
In **production**, `BONDERY_PRIVATE_REDIS_URL` is **required**. See [Self-hosting](../deploy/get-started.mdx).
...
> **Before using these commands**, complete environment setup for each app in the steps above.
...
Port registry: [architecture.md](architecture.mdx#apps) (`npm run check-dev-ports` in CI).

| App | Source | Dev port | Start command (from app folder) |
|---|---|---|---|
| Postgres | [`deploy/bondery`](../../deploy/bondery/docker-compose.dev-db.yml) | 54322 | `docker compose -f docker-compose.dev-db.yml up -d` |
