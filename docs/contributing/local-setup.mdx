...
| Command | Description |
|---|---|
| `npm run db:migrate:deploy -w @bondery/db` | Apply pending Prisma migrations (local dev, CI) |
| `npm run release-migrate -w @bondery/db` | Full release gate: migrate + functions.sql + OAuth + platform admins (CI) |
