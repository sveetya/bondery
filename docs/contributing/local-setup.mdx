...
Set `DATABASE_URL` and `BONDERY_PRIVATE_POSTGRES_PASSWORD` in root `.env.local` (see [`.env.local.example`](../../.env.local.example)). OAuth client ids/secrets for Better Auth live under `BONDERY_PRIVATE_AUTH_*`. After editing, run `npm run env` to sync app env files.
...
Postgres listens on **host port 54322** (`postgresql://postgres:<password>@127.0.0.1:54322/bondery` — password must match `BONDERY_PRIVATE_POSTGRES_PASSWORD`).
