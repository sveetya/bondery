...
| Command | Description |
|---|---|
| `npm run db:migrate:deploy -w @bondery/db` | Apply pending Prisma migrations |
| `npm run db:migrate:dev -w @bondery/db` | Author new migrations interactively (requires shadow DB with PostGIS) |
| `npm run db:studio -w @bondery/db` | Open Prisma Studio against `DATABASE_URL` |
| `npm run db:functions -w @bondery/db` | Apply raw SQL functions (included in dev bootstrap) |
| `npm run provision-oauth-clients:dev -w api` | Upsert first-party OAuth clients (included in dev bootstrap) |
...
npm run bootstrap:seaweedfs      # optional — API dev boot also ensures buckets
```
...
<Callout type="warn">
**Avatar previews need public bucket policy.** Bootstrap applies anonymous `GetObject` on `avatars`. If uploads succeed but images 404/403 in the browser, run `npm run bootstrap:seaweedfs` or restart the API (dev bootstrap).
</Callout>
...
npm run compile:packages       # one-shot package compile
```
...
Port registry: [architecture.mdx](architecture.mdx#apps) (`npm run check-dev-ports` in CI).
...
| `api` | [`apps/api`](../../apps/api) | 26631 | `npm run dev` (in app) or `npm run dev:webapp-api` |
| `webapp` | [`apps/webapp`](../../apps/webapp) | 26632 | `npm run dev` (in app) or `npm run dev:webapp-api` |
| `website` | [`apps/website`](../../apps/website) | 26630 | `npm run dev` |
| `mobile` | [`apps/mobile`](../../apps/mobile) | 26634 (Expo Metro) | `npm run dev` |
