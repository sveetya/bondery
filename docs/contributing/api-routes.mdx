...
`npm run check:contracts` validates every registered example with `schema.parse(example)`. `npm run check:openapi-spec -w api` fails if any `2xx` or `4xx`/`5xx` `application/json` response lacks an `example` or has an empty schema, and if any `POST`/`PUT`/`PATCH` JSON request body lacks an `example`.
...
CI: `npm run check:route-security -w api` (wired into `check:types`). Boot-time audit: `route-security-audit.test.ts` in `test:api`.
...
|---------|----------------|
| `npm run check-api-schema-patterns -w api` | Route files have descriptions, responses, `FastifyZodOpenApiSchema` |
| `npm run check:route-security -w api` | No auth hooks in routes; mount table covers all plugins; non-plugin files listed in `route-non-plugin-files.json` |
...
| `npm run check-route-errors -w api` | Bans legacy error patterns; requires `code` on DomainError |
| `npm run check:contracts` | Schema boundary assertions + OpenAPI example validation (`check-contracts-openapi-examples.ts`) |
...
`npm run check-route-errors -w api` bans `handleDomainError`, `reply.status(5xx)` in routes, `Error + statusCode` in auth, and `DomainError` without `code`.
