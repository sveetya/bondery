...
| API contract | Zod schemas in `@bondery/schemas` → generated `apps/api/openapi.yaml` → Fumadocs OpenAPI |
...
<Files>
  ...
  <File name="apps/api/openapi.yaml (committed artifact)" />
  <File name="Fumadocs OpenAPI block in docs" />
</Files>
...
| Raw SQL | `npm run db:functions` for Postgres functions |
...
After schema changes, commit the new migration files and regenerate the Prisma client (`npm run db:generate` runs on install).

---

<Callout type="info">
**Fun fact:** Bondery local apps use consecutive ports in the `2663x` block — phone keypad **B-O-N-D** → **2-6-6-3** ("Dial BOND"). Local Postgres uses **54322** (direct connection, no gateway). Local API Redis uses **26636** (`npm run start:redis`). Port constants live in [`packages/schemas/src/constants/dev-ports.ts`](../../packages/schemas/src/constants/dev-ports.ts).
</Callout>
...
**Local dev:** port `26639` · `npm run dev:emails` → [http://localhost:26639](http://localhost:26639). Runs alongside the Chrome extension dev server (`26633`) without a port conflict.
