...
<Callout type="info">
**Fun fact:** Bondery local apps use consecutive ports in the `2663x` block — phone keypad **B-O-N-D** → **2-6-6-3** ("Dial BOND"). Local Postgres uses **54322** (direct connection, no gateway). Optional local API Redis uses **26636** (`npm run start -w redis`). Port constants live in [`packages/schemas/src/constants/dev-ports.ts`](../../packages/schemas/src/constants/dev-ports.ts); CI enforces them with `npm run check-dev-ports`.
</Callout>
