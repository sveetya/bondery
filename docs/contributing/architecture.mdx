...
React Email templates rendered and sent by the API. Files live in `src/templates/{account,billing,internal,notifications}/` and share `EmailWrapper` / `EmailBody` chrome. Import from `@bondery/emails`. Details: [`.agents/skills/bondery-emails`](../../.agents/skills/bondery-emails/SKILL.md).
