...
| `npm run generate-env-examples` | Regenerate `.env*.example`, `deploy/bondery/.env.example`, and `turbo.json` from the manifest |
...
Templates: [`.env.local.example`](../../.env.local.example) (local dev) · [`deploy/bondery/.env.example`](../../deploy/bondery/.env.example) (self-host compose).
...
[`@bondery/helpers/env`](../../packages/helpers/src/env/manifest.ts) is the single contract: canonical keys, which apps receive them, `requiredIn`, deploy operator examples (`deployExample`), and turbo scopes. Per-app `check-env` scripts read required lists from the manifest (API also cross-checks `required-env.ts`).
