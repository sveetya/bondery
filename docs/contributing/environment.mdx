...
Operators copy [`deploy/bondery/.env.example`](../../deploy/bondery/.env.example) to `deploy/bondery/.env` for Compose — same `BONDERY_*` names; Compose derives `DATABASE_URL` and public URLs from domains. The example file is **generated** from the manifest (`deployExample` metadata) via `pnpm run env:sync`. See [Installation](../deploy/installation.mdx). Bondery production deployment on Dokploy is documented in [Dokploy deployment](./dokploy.mdx) (contributors / operators only).
