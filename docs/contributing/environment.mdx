...
[`.infisical.json`](../../.infisical.json) is committed. It contains **no secrets** — only the Infisical project id, EU domain, and environment slug. Use `development` for local `env:pull`, `staging` for release smoke, and `production` for Dokploy ops sync. Forks can copy [`.infisical.json.example`](../../.infisical.json.example) and set a different `projectId`.
