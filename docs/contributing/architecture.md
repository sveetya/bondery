...
| Layer | Role |
|---|---|
| **Postgres** | Source of truth for all user data (Prisma + SQL functions in `packages/db`) |
| **Fastify API** | Better Auth, business logic, list/search endpoints, imports, mobile sync (push/pull/bootstrap) |
| **Clients** | Webapp, mobile, and Chrome extension — OAuth via Better Auth on the API; app data via the API |

Better Auth on the API issues sessions and access tokens. Clients attach those tokens to API requests. The API uses Prisma for all Postgres reads and writes.
...
│   └── redis/            # Local dev Redis (Docker)
├── packages/
│   ├── db/               # Prisma schema, migrations, SQL functions
│   ├── schemas/          # Zod schemas, domain types, sync protocol
...
├── archive/              # Historical tooling (legacy Supabase CLI project)
...
| Authentication | Better Auth (BFF OAuth client + encrypted session cookie) |
...
Server components and client hooks call the Fastify API with the user's session or access token. A small set of Next.js route handlers (e.g. streaming chat) proxy to the API after verifying the session locally.
...
| Auth | [Better Auth](https://www.better-auth.com/) (OAuth provider, sessions, API keys) |
...
Every authenticated request validates the Better Auth session or API key. List endpoints follow a shared [pagination contract](../../.agents/skills/bondery-specific/references/api/api-design.md) (`limit`, `offset`, `search`, `sort`, nested `pagination` object).
...
| Authentication | Better Auth OAuth (`/auth/oauth2/*` on the API) via `chrome.identity` |
...
PostgreSQL schema, Prisma migrations, and SQL functions (extensions, RPCs, triggers).

| Concern | Technology |
|---|---|
| Database | [PostgreSQL](https://www.postgresql.org/) + PostGIS |
| ORM | [Prisma](https://www.prisma.io/) |
| Migrations | `prisma migrate` + `scripts/apply-sql-functions.ts` |

**Local dev Postgres** (host port `54322`): `deploy/bondery/docker-compose.dev-db.yml`

---

{% hint style="info" %}
**Fun fact:** Bondery local apps use consecutive ports in the `2663x` block — phone keypad **B-O-N-D** → **2-6-6-3** ("Dial BOND"). Local Postgres uses **54322**. Optional local API Redis uses **26636** (`npm run start -w redis`). Port constants live in [`packages/schemas/src/constants/dev-ports.ts`](../../packages/schemas/src/constants/dev-ports.ts); CI enforces them with `npm run check-dev-ports`.
{% endhint %}
...
| [`packages/db`](https://github.com/usebondery/bondery/tree/main/packages/db) | Prisma schema, migrations, generated client, SQL functions |
...
Single source of truth for Zod schemas, inferred TypeScript types, and the mobile sync protocol (`src/sync/`). Import domain types from here — do not redefine them in apps.
...
**API internals:** Route handlers are thin. `domains/*` functions accept a `DomainContext` (user, Prisma) and return `{ data, txid }` where sync confirmation is required.
