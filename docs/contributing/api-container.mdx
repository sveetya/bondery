---
title: "API container (GHCR + external Redis)"
icon: Container
---

<Callout type="info">
**Bondery production / advanced operators** — self-hosters use the unified [`deploy/bondery`](../../deploy/bondery/) stack (webapp + API + Redis + Postgres). See [Installation](../deploy/installation.mdx).
</Callout>
...
Local laptop Redis: [`deploy/bondery/docker-compose.dev-redis.yml`](../../deploy/bondery/docker-compose.dev-redis.yml) (`pnpm run start:redis`, port 26636) — not the production `deploy/bondery` stack.
...
| Layer | Example | Role |
|-------|---------|------|
| **Git tag** | `v1.7.1` | Triggers unified [`release.yml`](../../.github/workflows/release.yml) |
| **Semver image tag** | `1.7.1` | Immutable artifact — **pin production via `BONDERY_INFRA_VERSION`** (api + webapp) |
...
| Production | Git tag `vX.Y.Z` on `release` | `X.Y.Z`, `production` | Live api + webapp (unified release) |

**Compose defaults:** omit `BONDERY_INFRA_VERSION` to pull the floating `production` channel. Set `BONDERY_INFRA_VERSION=X.Y.Z` to pin **both** api and webapp — see [`deploy/bondery/.env.example`](../../deploy/bondery/.env.example).
...
|--------|----------|---------|
| `BONDERY_OPS_GHCR_WRITE_TOKEN` | Yes | Push/pull images on GHCR — loaded from Infisical production in CI (`ghcr-login-infisical`) |
...
3. CI publishes `ghcr.io/usebondery/api:1.7.1` and `ghcr.io/usebondery/webapp:1.7.1`, promotes `:production` after smoke.
4. Set `BONDERY_INFRA_VERSION=1.7.1` in Dokploy and redeploy api + webapp together.

See the [`bondery-release`](../../.agents/skills/bondery-release/SKILL.md) skill (`dokploy-pins` reference) for the full stack pin workflow.
...
Full Dokploy runbook: [dokploy.md](./dokploy.mdx).
