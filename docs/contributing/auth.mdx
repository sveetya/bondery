...
# Better Auth signing (if not already set)
openssl rand -hex 32   # BONDERY_PRIVATE_BETTER_AUTH_SECRET
```
...
Requires Postgres running with migrations and provisioning complete. Starts the API in-process and exercises the full authorization-code + PKCE flow.
...
| --- | --- | --- | --- |
| `BONDERY_PRIVATE_BETTER_AUTH_SECRET` | Yes | API | Signs Better Auth sessions and tokens (≥32 chars) |
