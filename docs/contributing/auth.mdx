...
OAuth clients are **not** registered through a UI or a logged-in session. `apps/api/scripts/provision-oauth-clients.ts` upserts first-party clients in Postgres from env vars. It runs automatically after migrations in CI and self-host deploys (`packages/db/scripts/release-migrate.ts`).
