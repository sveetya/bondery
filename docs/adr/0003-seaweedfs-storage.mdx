---
title: "ADR 0003: SeaweedFS object storage (S3-compatible)"
hidden: true
---
...
2. API uses a single **`S3Storage`** implementation; `getStorage()` requires S3 env vars (no `BONDERY_STORAGE_DRIVER`). The API never calls SeaweedFS `/status`; the S3 API is the provider contract for a future swap.
