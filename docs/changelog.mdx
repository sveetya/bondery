...
- API, webapp, mobile, and Chrome extension now authenticate through Better Auth instead of Supabase Auth.
...
- **Auth and database stack:** Supabase Auth and the Supabase JS client are removed. Self-hosters must run `npm run setup:dev`, apply Prisma migrations, and configure Better Auth OAuth secrets per [environment configuration](contributing/environment.mdx).
