...
- Chrome extension: Version check reads `GET /extension/manifest` on the API host. Minimum Chrome extension version bumped to 1.7.5.
...
- Monorepo package manager migrated from npm to pnpm 11; Docker builds use `pnpm fetch` with BuildKit store cache mounts.
